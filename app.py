                ...
                dbc.ButtonGroup(
                    children=[
                        dbc.Button(
                            [
                                html.I(
                                    className=f"fas fa-chess-{name.lower()} mr-2"),
                                name,
                            ],
                            color="primary",
                            n_clicks=0,
...
dropdown_status = dbc.DropdownMenu(
    [
        ...
        dbc.DropdownMenuItem(str("Checkmate").upper(),
                             id="st_mate", n_clicks=0),
        dbc.DropdownMenuItem(str("Resignation").upper(),
                             id="st_resign", n_clicks=0),
        dbc.DropdownMenuItem(
...
dropdown_winner = dbc.Collapse(
    dbc.DropdownMenu(
        [
            ...
            dbc.DropdownMenuItem(str("White").upper(),
                                 id="wn_white", n_clicks=0),
            dbc.DropdownMenuItem(str("Black").upper(),
                                 id="wn_black", n_clicks=0),
        ],
        label="Winning Side",
        id="dropdown_winner",
    ),
...
dropdown_time_control = dbc.DropdownMenu(
    [
        ...
        dbc.DropdownMenuItem(str("Bullet").upper(),
                             id="tc_bullet", n_clicks=0),
        ...
        dbc.DropdownMenuItem(str("Classical").upper(),
                             id="tc_classic", n_clicks=0),
        dbc.DropdownMenuItem(str("No Time Control").upper(),
                             id="tc_none", n_clicks=0),
    ],
    label="Time Control",
...
dropdown_game_type = dbc.DropdownMenu(
    [
        ...
        dbc.DropdownMenuItem(str("Tournament").upper(),
                             id="gt_tourney", n_clicks=0),
    ],
    label="Game Type",
...
def update_chessboard(
    ...
    df["rows"] = df["rows"].replace(
        {i: list(range(8))[::-1][i] for i in range(8)})
